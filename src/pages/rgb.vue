  ...
  <div ref="page" class="demo-flow">
    <VueFlow v-model="elements" @pane-ready="onLoad">
      <template #node-rgb="props">
        <DesignerRgbNode v-bind="props" :amount="color" @change="onChange" />
      </template>
      <template #node-rgb-output="props">
        <DesignerRgbOutputNode v-bind="props" :rgb="`rgb(${color.red}, ${color.green}, ${color.blue})`" />
      </template>
    </VueFlow>
  </div>
</template>
...
import { Elements, FlowInstance, VueFlow } from "~/packages/designer";
